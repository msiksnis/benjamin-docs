...
- Final whole-branch review: committed filename enumeration is explicitly bounded at 64 MiB and propagates typed failure into fail-closed readiness; publication scanning covers punctuation-delimited macOS/Linux/Windows homes; repository-only strict doctor never reads integration paths; and hook health requires the exact target session-start command while reporting legacy stop hooks separately.
