...
- Agent session hook installation lives in `src/hooks.ts`; shared-schema Claude Code and Codex health requires an executable `type: command` entry inside `SessionStart[].hooks[]`, the exact target format command, and the supported `startup|resume|clear` matcher. Reinstalling a target removes malformed, wrong-matcher, wrong-format, and legacy Benjamin entries, adds the current start entry, and preserves user commands plus mixed-group fields. Legacy Benjamin stop hooks are reported separately and make strict target diagnosis fail until migration. Bounded session-start context, the silent legacy stop adapter, and explicit stop-diagnostic wording live in `src/session.ts`; local per-session dirty-tree baselines live in `src/session-state.ts`.
