...
The 0.12.0 upgrade boundary is one command per initialized repository after the package itself is updated. Plain `bd upgrade` refreshes Benjamin-owned project metadata, agent guidance, the current skill bundle, existing Memory Views, and all Claude Code, Codex, and Cursor session-start hooks. It removes legacy Benjamin stop hooks while preserving user-owned configuration. No separate hook command is required; `bd upgrade --no-hooks` is the explicit environment opt-out.
...
- Shared git history helpers live in `src/git.ts`; committed-history drift detection lives in `src/drift.ts`. Filename enumeration uses an explicit 64 MiB child-process bound rather than Node's default buffer. Git child processes force `LC_ALL=C` and `LANG=C` while preserving the caller environment, so legitimate unavailability classification does not depend on the machine locale. `getChangedFiles` distinguishes the legitimate `not_git_repository` and `no_head` unavailable states from execution/enumeration failures. The unavailable states remain usable only in planning mode; typed failures block both `committed_freshness` and `working_tree_impact`, including in planning mode. Successful large analyses keep complete internal file accounting while readiness output samples paths. Session-start drift reads last-doc commits in one batched Git query and caches identical diff/count queries so the hook stays within its latency budget without changing per-doc drift semantics.
- Agent session hook installation lives in `src/hooks.ts`; shared-schema Claude Code and Codex health requires an executable `type: command` entry inside `SessionStart[].hooks[]`, the exact target format command, and the supported `startup|resume|clear` matcher. Reinstalling a target removes malformed, wrong-matcher, wrong-format, misplaced, and legacy Benjamin entries, adds or reuses the current start entry, and preserves unrelated group properties and nested user hooks exactly. Ownership requires `benjamin-docs session-start` or `benjamin-docs session-stop` at command start followed by whitespace or end, so shell wrappers, prefixes, and logging text that merely mention those strings remain user-owned during repair and uninstall. Enumeration is also schema-local: Claude/Codex inspect only a direct group command or direct `group.hooks[]` entries under `SessionStart` and `Stop`; Cursor inspects only direct entries under `sessionStart` and `stop`. In those locations, only the exact target `session-start` is healthy under the start event, while any direct Benjamin start or stop command under the stop event is unsafe and makes status/strict doctor unhealthy until repair. Arbitrary nested custom metadata and other hook-event names are preserved and ignored for health, repair classification, migration, and uninstall. Direct Benjamin commands in shared-schema mixed groups are removed as single properties or entries; Cursor's flat schema instead removes the whole Benjamin-owned entry so no commandless remnant survives. Parseable files with incompatible user-owned `hooks` or relevant event shapes are skipped unchanged and fail the required upgrade migration instead of being normalized destructively. Legacy Benjamin stop hooks retain their specific diagnostic, while other unsafe Stop-event commands use the broader unsafe-command diagnostic. Bounded session-start context, the silent legacy stop adapter, and explicit stop-diagnostic wording live in `src/session.ts`; local per-session dirty-tree baselines live in `src/session-state.ts`.
- The final hook invariant is stricter than the original single-match wording: a healthy target has exactly one canonical start command and no additional direct Benjamin command in its start event. Leading whitespace still denotes a directly executable Benjamin command; wrappers and prefixes do not. Status, strict doctor, install, and uninstall share the same structural preflight. Unknown Cursor schema versions, primitive relevant-event entries, non-array shared group hooks, and primitive nested hook entries are incompatible and remain unchanged. Hook reads, replacement, and deletion reject symlinked ancestors. Existing hook files are replaced through a same-directory temporary file that is flushed, retains POSIX mode/owner metadata, and is atomically renamed; failures leave the original directory entry intact and cleanup cannot mask the primary error. Raw bytes from the read are checked immediately before rename or deletion to catch visible stale-state conflicts; this narrows the race window but is explicitly not cross-process compare-and-swap.
...
- Publication policy lives only in `src/export-policy.ts`. `src/export.ts` renders candidate snapshots in memory and must pass both source docs and every byte of the final artifact through the side-effect-free preflight before preparing, cleaning, or writing an export directory. Customer features can pass only with repository readiness, non-private sources, required customer sections, and a `Checked:` record naming a typed `file:`, `route:`, `component:`, `mutation:`, `rpc:`, `test:`, `command:`, or `manual:` target plus a meaningful observed `Result:`; unsupported customer/public operations fail without writes. The path scanner parses local `file:` URLs, including empty-host and `localhost` forms, and recognizes macOS, Linux, and case-insensitive Windows drive/`Users` home paths with either separator across generated metadata, titles, Markdown links, quotes, backticks, and punctuation while avoiding ordinary web URLs and relative paths. Scope titles must be non-empty single-line metadata, and all generated frontmatter values are JSON-quoted YAML scalars. Generated metadata says verification was recorded, not that deterministic code proved semantic truth.
- Skill installation and Claude zip packaging live in `src/install-skill.ts` and `src/package-skill.ts`. Skill refresh preflights every selected bundle destination before reading or writing the bundle, rejecting symlinked files or ancestors, non-directory parents, and existing paths that resolve outside the resolved Benjamin Docs home. Plain upgrade preflights all four targets together so an unsafe later target cannot leave earlier partial skill writes.
...
Version 0.10.0 introduced consent-based hooks for Claude Code, Codex, and Cursor. The 0.12.0 trust-foundation behavior installs only `bd session-start`, which injects bounded context containing the memory location, read-first docs, and drift summary. Existing `bd session-stop` commands remain accepted but produce no output, so they cannot block completion or auto-submit follow-up work. Hook files are user-owned: installation or upgrade migrates only legacy Benjamin stop entries, uninstall removes only marked Benjamin commands even inside mixed groups, and every user command, custom field, and unparseable file is preserved. Interactive `init` still asks for consent; after the user updates the package, plain `upgrade` refreshes hooks by default and `upgrade --no-hooks` is the explicit opt-out.
...
The CLI treats generated paths as project-relative paths. It rejects absolute paths, traversal, unsafe path segments, symlinked generated targets or ancestors, and metadata or docs that resolve outside the project root. Validation repeats those checks for existing metadata, managed Markdown docs, markdown links, anchors, and scope paths. User-owned hook settings use the same containment checks plus failure-safe atomic replacement.
