...
- Agent session hook installation lives in `src/hooks.ts`; shared-schema Claude Code and Codex health requires an executable `type: command` entry inside `SessionStart[].hooks[]`, the exact target format command, and the supported `startup|resume|clear` matcher. Reinstalling a target removes malformed, wrong-matcher, wrong-format, and legacy Benjamin entries, adds or reuses the current start entry, and preserves unrelated group properties and nested user hooks exactly. Ownership requires `benjamin-docs session-start` or `benjamin-docs session-stop` at command start followed by whitespace or end, so shell wrappers, prefixes, and logging text that merely mention those strings remain user-owned during repair and uninstall. Enumeration is also schema-local: Claude/Codex inspect only a direct group command or direct `group.hooks[]` entries under `SessionStart` and `Stop`; Cursor inspects only direct entries under `sessionStart` and `stop`. Arbitrary nested custom metadata and other hook-event names are preserved and ignored for health, repair classification, migration, and uninstall. Top-level Benjamin start or legacy stop commands in shared-schema mixed groups are removed as single properties. Cursor's flat schema instead removes the whole Benjamin-owned entry so no commandless remnant survives. Parseable files with incompatible user-owned `hooks` or relevant event shapes are skipped unchanged and fail the required upgrade migration instead of being normalized destructively. Legacy Benjamin stop hooks are reported separately and make strict target diagnosis fail until migration. Bounded session-start context, the silent legacy stop adapter, and explicit stop-diagnostic wording live in `src/session.ts`; local per-session dirty-tree baselines live in `src/session-state.ts`.
