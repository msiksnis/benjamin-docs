...
- Publication policy lives only in `src/export-policy.ts`. `src/export.ts` renders candidate snapshots in memory and must pass both source docs and every byte of the final artifact through the side-effect-free preflight before preparing, cleaning, or writing an export directory. Customer features can pass only with repository readiness, non-private sources, required customer sections, and a concrete `Checked: ...; Result: ...` evidence record; unsupported customer/public operations fail without writes. The path scanner recognizes local `file:` URIs plus macOS, Linux, and case-insensitive Windows drive/`Users` home paths with either separator across generated metadata, titles, Markdown links, quotes, backticks, and punctuation while avoiding ordinary web URLs and relative paths. Generated metadata says verification was recorded, not that deterministic code proved semantic truth.
