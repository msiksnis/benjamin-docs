...
- `package.json` owns the npm version, package description, keywords, published files, bin alias, and release scripts. Version `0.12.0` is the current published package.
...
- `test/readiness-report.test.ts` reproduces false-ready, cross-stack, deletion, planning-mode, non-Git/unborn-HEAD availability, fail-closed Git analysis, guidance, and an 8,200-file oversized committed diff. The oversized-diff cleanup and maximum-length custom-directory stress fixtures run on Unix hosts, while the Windows matrix covers the functional readiness and session-context behavior without unreliable filesystem-limit fixtures. `test/export-policy.test.ts` and `test/validate-export.test.ts` cover the shared preflight, Markdown-delimited cross-platform home paths including mixed-case Windows forms, and verified customer success path. `test/doctor.test.ts` covers target isolation, unreadable global state, exact target hook commands, stop-only legacy hooks, and diagnose-install-healthy repair for every hook target, including mixed direct-command groups, exact user-data preservation, duplicate avoidance, and idempotent reinstall for Claude Code and Codex; `test/mcp.test.ts` enforces task-context bounds.
...
- `src/install-skill.ts` owns stable slash-normalized display paths. `test/helpers.ts`, `test/doctor.test.ts`, and `test/mcp.test.ts` contain the Windows-safe temporary-workspace and child-process lifecycle fixtures required by the cross-platform matrix.
