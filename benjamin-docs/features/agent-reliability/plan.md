...
2026-07-09 update: session hooks and drift detection moved to the Drift And Session Hooks scope and shipped in 0.10.0; validated memory-write tooling moved to the MCP Memory Server scope for 0.11.0. Remove both from this plan's open items.
...
11. Fix stop-hook turn safety with per-session content baselines, fail-open recovery, and an answer-preserving continuation contract. Published in 0.11.1 and verified with the installed registry build.
