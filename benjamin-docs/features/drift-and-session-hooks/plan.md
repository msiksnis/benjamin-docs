...
- `src/session-state.ts`: 0.11.1 hotfix state machine for content-fingerprinted dirty-tree baselines, per-session isolation, fail-open recovery, pending-nudge acknowledgement, and seven-day cache pruning.
...
- `test/drift-hooks.test.ts` (23 tests): drift lifecycle, hook preservation, session command formats, pre-existing dirty-file suppression, new changes to already-dirty files, answer-preserving feedback, fail-open recovery, session isolation, memory-update suppression, and cache expiry.
