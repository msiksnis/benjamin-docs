...
- `bd hooks install` wires session hooks for Claude Code, Codex, and Cursor. Session start injects compact memory context (read-first docs plus drift summary) via `bd session-start`; stop nudges once via `bd session-stop` when source changed without a memory update.
...
In scope: committed-history drift detection, hook install/status/uninstall with strict preservation of user-owned hook file content, session start/stop commands with per-tool output formats.

Out of scope: MCP server (next release), per-session state tracking, hooks for tools without a session-start mechanism.
