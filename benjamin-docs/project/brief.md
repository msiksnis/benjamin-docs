...
Final whole-branch review hardening preserves those boundaries under large and hostile inputs: committed filename enumeration is explicitly bounded; real Git failures block both freshness dimensions while non-Git and unborn-HEAD planning stay usable; publication path scanning is Markdown-, platform-, and Windows-case-aware; repository-only strict checks never inspect integration state; and exact target reinstall repairs stale Benjamin hooks while preserving user-owned hook data. `upgrade` remains advanced so the primary human surface is still exactly four commands.
