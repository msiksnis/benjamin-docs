...
`benjamin-docs` is a published local-first CLI for turning useful AI chats into durable project docs. The package is public as `benjamin-docs`, currently published at `0.9.0`, and focuses on local files rather than SaaS.
...
The current publish target is 0.9.1: the first Agent Reliability patch on top of guided export. BD should turn maintained project memory into concise generated snapshots without making humans learn many commands, and agents should record implementation verification before customer-facing exports.
