...
The final branch review also verified the awkward edge cases: very large committed diffs cannot silently look current, local home paths are rejected even inside ordinary Markdown punctuation, repository health does not depend on readable global tool state, and a leftover stop-only hook is not mistaken for a healthy session-start integration. The visible human command set remains `init`, `ready`, `export`, and `help`.
