...
`benjamin-docs` is a published local-first CLI for turning useful AI chats into durable project docs. The package is public as `benjamin-docs`, currently published at `0.4.2`, and focuses on local files rather than SaaS.
...
The current quality focus is 0.5.0: Continuation Proof. BD should turn messy chats, existing repos, and feature plans into docs that a future human or agent can actually continue from without reading the original conversation.
