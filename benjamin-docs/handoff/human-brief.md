...
`benjamin-docs` is a published local-first CLI and agent skill for persistent project memory. The package is public as `benjamin-docs`; `0.9.3` is currently published on npm, and `0.10.0` is implemented and prepared as the next publish. The product focuses on local files rather than SaaS.
