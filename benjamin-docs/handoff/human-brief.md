...
`benjamin-docs` is a published local-first CLI and agent skill for persistent project memory. The package is public as `benjamin-docs`; `0.9.2` is currently published on npm, and `0.9.3` is prepared as the next publish for the README/npm positioning update. The product focuses on local files rather than SaaS.
