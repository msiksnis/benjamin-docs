...
- Hardened the customer publication boundary after adversarial review: the complete in-memory artifact, including titles and generated metadata, is scanned before writes; local `file:` user-home URLs are parsed across empty-host and `localhost` forms; scope titles are single-line and generated frontmatter is safely quoted; vague evidence is rejected unless `Checked:` names a typed artifact/check and `Result:` records a meaningful outcome; generated metadata records verification evidence without claiming semantic proof.
...
- Skill refresh now fails before bundle I/O when any selected destination is symlinked, has a non-directory ancestor, or resolves outside the Benjamin Docs home; all-target upgrade preflight prevents partial writes to earlier skill targets. Hook repair and uninstall now recognize only anchored Benjamin session commands, preserving prefixed, wrapped, and logging commands that merely mention start or stop.
...
- Final review hardening: Git filename enumeration now has an explicit 64 MiB bound, typed failures block both committed freshness and working-tree impact even in planning mode, and readiness samples evidence so an 8,200-file diff remains accurate without oversized output. Only non-Git and unborn-HEAD planning repositories retain non-blocking unavailable dimensions.
- Publication path scanning now blocks quoted, backticked, linked, and punctuation-delimited macOS/Linux/Windows home paths, including mixed-case drive/`Users` forms and both separators, while avoiding ordinary URLs and relative paths.
- Repository-only `doctor --strict` loads project state before and independently from integrations, so unreadable global state cannot break the repository gate. Shared-schema hook health now requires the supported matcher and an executable exact-format command inside `SessionStart[].hooks[]`; target reinstall removes only a malformed top-level Benjamin command, preserves unrelated group fields and nested user hooks exactly, and reuses an existing valid nested Benjamin entry without duplication. Git subprocesses force stable English diagnostics without dropping caller environment variables.
