...
- Repository-only `doctor --strict` loads project state before and independently from integrations, so unreadable global state cannot break the repository gate. Target hook health requires the exact session-start event/format and reports legacy stop registrations separately; `hooks install --target` replaces stale Benjamin starts and stops while preserving user commands and custom group fields.
